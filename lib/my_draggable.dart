...
typedef void MyDraggableCanceledCallback(Velocity velocity, ui.Offset offset);
...
class MyDraggable<T> extends StatefulWidget {
  ...
  const MyDraggable({
    ...
    this.feedbackOffset = ui.Offset.zero,
  ...
  final ui.Offset feedbackOffset;
  ...
  final ui.VoidCallback? onDragStarted;
  ...
  /// need to check [State.mounted] to check whether the state receiving the
  /// callback is still in the tree.
  final ui.VoidCallback? onDragCompleted;


  final ValueChanged<ui.Offset>? onMove;
...
class MyDraggableState<T> extends State<MyDraggable<T>> {
  ...
  DragAvatar<T>? _startDrag(ui.Offset position) {
    if (widget.maxSimultaneousDrags != null && _activeCount >= widget.maxSimultaneousDrags!)
      return null;
    ui.Offset dragStartPoint;
    switch (widget.dragAnchor) {
      case DragAnchor.child:
        final RenderObject? renderObject = context.findRenderObject();
        dragStartPoint = (renderObject as RenderBox).globalToLocal(position);
        break;
    ...
    final DragAvatar<T> avatar = new DragAvatar<T>(
        ...
        onDragEnd: (Velocity velocity, ui.Offset offset, bool wasAccepted) {
          if (mounted) {
            setState(() {
              _activeCount -= 1;
            });
...
class _MyDragTargetState<T> extends State<MyDragTarget<T>> {
  ...
  bool didEnter(DragAvatar<dynamic> avatar) {
    assert(!_candidateAvatars.contains(avatar as DragAvatar<T>));
  ...
  void didLeave(DragAvatar<dynamic> avatar) {
    assert(_candidateAvatars.contains(avatar as DragAvatar<T>) || _rejectedAvatars.contains(avatar));
    if (!mounted)
      return;
    setState(() {
      _candidateAvatars.remove(avatar as DragAvatar<T>);
...
typedef _OnDragEnd = void Function(Velocity velocity, ui.Offset offset, bool wasAccepted);
...
class DragAvatar<T> extends Drag {
  DragAvatar({
    ...
    this.dragStartPoint = ui.Offset.zero,
    this.feedback,
    this.feedbackOffset = ui.Offset.zero,
    this.onDragEnd,
    this.onMove,
  ...
  final T? data;
  final ui.Offset dragStartPoint;
  final Widget? feedback;
  final ui.Offset feedbackOffset;
  ...
  late ui.Offset _position;
  late ui.Offset _lastOffset;
  OverlayEntry? _entry;
  ...
  void updateDrag(ui.Offset globalPosition) {
    ...
    // Enter new targets.
    _MyDragTargetState<T>? newTarget;
    for (final _MyDragTargetState<T> target in targets) {
  ...
  Iterable<_MyDragTargetState<T>> _getMyDragTargets(List<HitTestEntry> path) sync* {
    ...
    for (HitTestEntry entry in path) {
      ...
      if (target is RenderMetaData) {
        if (target.metaData is _MyDragTargetState<T>)
  ...
  Widget _build(BuildContext context) {
    final RenderObject? box = overlayState.context.findRenderObject();
    final ui.Offset overlayTopLeft = (box as RenderBox).localToGlobal(ui.Offset.zero);
